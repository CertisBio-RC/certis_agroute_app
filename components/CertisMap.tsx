...
// --- Shared category palette for UI + map (also imported by page.tsx) ---
...
// Satellite streets for hybrid, streets for street
...
// Build a Mapbox expression to color points by `category`
...
// Filter expression for “kingpins” using common keys
...
const CertisMap: React.FC<CertisMapProps> = ({ styleMode, categories, onAddStop }) => {
  ...
  // Compute which categories are enabled
  const enabledCats = useMemo(() => {
    const on = Object.entries(categories)
      .filter(([, v]) => !!v)
      .map(([k]) => k);
    return new Set(on);
  }, [categories]);
  ...
  useEffect(() => {
    ...
    // Whenever style loads (initial + after style changes) add sources/layers
    const wireLayers = () => {
      if (!map.getSource(MAIN_SRC)) {
        map.addSource(MAIN_SRC, {
          ...
          clusterMaxZoom: 12,
          clusterRadius: 50
        } as any); // v3: use plain object; avoid v2-only types
      }

      // Cluster bubbles (light-blue) under everything
      ...
      // Cursor + hover popup for points & kingpins
      const showPopup = (e: mapboxgl.MapMouseEvent) => {
        const f = e.features && e.features[0];
        if (!f) return;
        const p = (f.properties || {}) as Record<string, any>;
        const name =
          p.name || p.Name || p.Retailer || p['Retailer Name'] || p['Store'] || 'Location';
        const addr =
          p.address || p.Address || p['Address 1'] || p['Address'] || '';
      ...
      // Clicking a cluster zooms in (use safe cast for v3 types)
      ...
      // Clicking a point calls optional onAddStop
      ...
      map.on('click', KING_LAYER, (e) => {
        ...
        const c = (f.geometry as any).coordinates as [number, number];
        const p = (f.properties || {}) as Record<string, any>;
        const name = p.name || p.Name || 'Kingpin';
        onAddStop?.({ name, coord: [c[0], c[1]] });
      });
    };
  ...
  // When the styleMode changes, switch style and re-wire on the next style.load
  useEffect(() => {
    const m = mapRef.current;
    if (!m) return;
    const next = styleUrlFor(styleMode);
    m.setStyle(next);
    m.once('style.load', () => {
      try {
        m.setProjection({ name: 'mercator' as any });
      } catch {/* ignore */}
      // Re-wire layers after a style swap
      // (sources/layers are cleared when style changes)
      // The effect above will also run on initial mount; here we repeat wire-up post-style swap.
      // We simply call the same logic by re-triggering data application below.
      // (No-op if sources already exist.)
      // eslint-disable-next-line @typescript-eslint/no-empty-function
    });
  }, [styleMode]);

  // Push data (and filters) into source when data or categories change
  useEffect(() => {
    const m = mapRef.current;
    if (!m) return;

    // Ensure source exists (if style just swapped)
    ...
    // Filter client-side before pushing to the source for snappier rendering
    const feats = (fc.features || []).filter((f) => {
      const p = (f.properties || {}) as Record<string, any>;
      const cat = p.category || p.Category || p.type || p.Type;
      if (!cat) return true; // if unknown category, keep it visible
    ...
    // Make sure layers exist after style swaps
    const needLayers = !m.getLayer(KING_LAYER) || !m.getLayer(MAIN_POINTS) || !m.getLayer(CLUSTER_LAYER);
    if (needLayers) {
      // Re-run initial layer wiring by programmatically reloading style (trigger style.load handler)
      // but gentler: just add missing layers here
