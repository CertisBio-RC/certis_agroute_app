  ...
  while (improved) {
    ...
    for (let i = 0; i < out.length - 2; i++) {
      for (let k = i + 1; k < out.length - 1; k++) {
        const candidate = [...out.slice(0, i), ...out.slice(i, k + 1).reverse(), ...out.slice(k + 1)];
        if (lengthOf(candidate) + 1e-6 < lengthOf(out)) {
          out.splice(0, out.length, ...candidate);
          improved = true;
        }
      }
    }
...
export function buildGoogleMapsLink(originLatLng: string, coords: Position[]): string {
  ...
  const params = new URLSearchParams({ api: "1", origin: originLatLng, destination });
  if (waypoints) params.set("waypoints", waypoints);
  return `https://www.google.com/maps/dir/?${params.toString()}`;
}

export function buildAppleMapsLink(originLatLng: string, coords: Position[]): string {
  const wp = coords.map((c) => `${c[1]},${c[0]}`).join(" to:");
  const params = new URLSearchParams({ saddr: originLatLng, daddr: wp || originLatLng });
  return `https://maps.apple.com/?${params.toString()}`;
}

export function buildWazeLink(_originLatLng: string, coords: Position[]): string {
  const next = coords[0]; if (!next) return "";
